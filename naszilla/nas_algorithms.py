...
DEFAULT_NUM_INIT = 10
DEFAULT_K = 10  # Q
DEFAULT_TOTAL_QUERIES = 100  # default=150
...
def run_nas_algorithm(algo_params, search_space, mp):
    ...
    if algo_name == 'random':
        data = random_search(search_space, **ps)  # 返回List[dict]记录搜索到的所有架构，其中每个dict代表一个架构
...
def compute_best_test_losses(data, k, total_queries, loss):
    """
    data记录整个NAS搜索到的所有架构。该函数返回在搜索过程中，每当刚评估完k个，2k个，3k个，....架构时得到的最优loss值
    ...
    results = []
    val_results = []
    for query in range(k, total_queries + k, k):  # query:已评估过的架构的个数
        best_arch = sorted(data[:query], key=lambda i:i[loss])[0]  # 前10,20,30,...个架构里，val loss最小的架构
        val_error = best_arch['val_loss']
        test_error = best_arch['test_loss']
        results.append((query, test_error))  # 评估完query个架构时，搜索到最好的test loss
        val_results.append((query, val_error))  # 评估完query个架构时，搜索到最好的valid loss

    return results, val_results


# modified 01-09
def record_best_data(data, k, total_queries):
    """
    data记录整个NAS搜索到的所有架构。该函数返回在搜索过程中，每当刚评估完k个，2k个，3k个，....架构时得到的最优loss值
    Given full data from a completed nas algorithm,
    output the test error of the arch with the best val error
    ...
    for query in range(k, total_queries + k, k):  # query:已评估过的架构的个数
        ...
        res['num_data'].append(query)                    # 数量上
        res['best_val_loss'].append(val_error)
        res['best_test_loss'].append(test_error)
...
def random_search(search_space,
    ...
    data = search_space.generate_random_dataset(num=total_queries, 
                                                ...
                                                cutoff=cutoff,
                                                deterministic_loss=deterministic)  # 得到List[dict]，其中每个dict记录搜索到的一个架构
    
    if verbose:
        top_5_loss = sorted([d[loss] for d in data])[:min(5, len(data))]  # 取valid loss最小的前5个
        print('random, query {}, top 5 losses {}'.format(total_queries, top_5_loss))    
    return data  # 得到List[dict]，其中每个dict记录搜索到的一个架构
...
def bananas(search_space, 
            metann_params,
            num_init=DEFAULT_NUM_INIT,  # 初始真实评估的架构个数
            k=DEFAULT_K,  # 每一轮BO真实评估的个数
            loss=DEFAULT_LOSS,
            total_queries=DEFAULT_TOTAL_QUERIES, 
            num_ensemble=5, 
    ...
    query = num_init + k  # 初始评估个数+本轮BO即将真实评估k个

    while query <= total_queries:

        xtrain = np.array([d['encoding'] for d in data])  # ndarray(D,40)
        ytrain = np.array([d[loss] for d in data])  # ndarray(D,)

        # 生成一组candicate架构 - get a set of candidate architectures
        candidates = search_space.get_candidates(data,
                                                 ...
                                                 loss=loss,
                                                 deterministic_loss=deterministic,
                                                 cutoff=cutoff)  # 从data中通过变异获取一些候选架构用于计算AF值

        xcandidates = np.array([c['encoding'] for c in candidates])  # ndarray(C,40)
        ...
        for e in range(num_ensemble):  # ensemble中的每一个NN模型

            if predictor == 'bananas':
                meta_neuralnet = MetaNeuralnet()
                net_params = metann_params['ensemble_params'][e]  # dict

                # 创建+训练一个NN模型
                train_error += meta_neuralnet.fit(xtrain, ytrain, **net_params)
        

                # 给出candidate的预测 predict the validation loss of the candidate architectures
                candidate_predictions.append(np.squeeze(meta_neuralnet.predict(xcandidates)))  # 添加一个元素 ndarray(C,)
        ...
        # “优化” AF - compute the acquisition function for all the candidate architectures
        candidate_indices = acq_fn(candidate_predictions, ytrain=ytrain, explore_type=explore_type)

        # 找出k个架构进行真实评估 - add the k arches with the minimum acquisition function values
